...
**Developer:** Ishaan Mandliya

While designing this platform, I prioritized idempotency using deterministic SHA256 hashing for assignments, ensuring users always receive the same variant regardless of retries or failures. I decoupled events from experiments so a single user action can count across multiple concurrent tests. The results endpoint uses query parameters to serve different stakeholders from one source of truth. JWT authentication keeps the API stateless and horizontally scalable.
